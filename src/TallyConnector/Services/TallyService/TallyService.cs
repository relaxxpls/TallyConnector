...
public partial class TallyService : ITallyService
{
    ...
    private static System.Text.RegularExpressions.Regex XmlTextRegex =
        new(@"(?<=>)(?!<)((.|\n)*?)(?=<\/[^>]+>|<[^>]+>)");
    private static System.Text.RegularExpressions.Regex XmlAttributeRegex =
        new(@"(?<=="")([^""]*)(?="")");
    private static System.Text.RegularExpressions.Regex XmlTextAsciiRegex = new(@"[\x00-\x1F]");
    private static System.Text.RegularExpressions.Regex XmlTextHexRegex =
        new(@"(?<=&#x)(.*?)(?=;)");
    ...
    public TallyService(string baseURL, int port, double timeoutMinutes = 3)
    {
        _httpClient = new();
    ...
    {
        TallyResult tallyResult = await SendRequestAsync(requestType: "Test Request", token: token);
    ...
    /// <inheritdoc/>
    public void Setup(string url, int port)
    {
    ...
    public async Task<LicenseInfo?> GetLicenseInfoAsync(CancellationToken token = default)
    {
        var LicenseInfo = await GetTDLReportAsync<LicenseInfo, LicenseInfo>(
            token: token,
            requestType: "License Info"
        );
        return LicenseInfo;
    }

    /// <inheritdoc/>
    public async Task<TallyResult> PostObjectToTallyAsync<ObjType>(
        ObjType Object,
        PostRequestOptions? postRequestOptions = null,
        string? objectType = null,
        CancellationToken token = default
    )
        where ObjType : TallyXmlJson, ITallyObject
    {
        if (Object is Voucher vch)
        {
            ...
            {
                vch.IsInvoice = true;
                var attributes = postRequestOptions.XMLAttributeOverrides[
                    typeof(ObjType),
                    "Ledgers"
                ];
                ...
                {
                    postRequestOptions.XMLAttributeOverrides.Add(
                        typeof(ObjType),
                        "Ledgers",
                        xmlattribute
                    );
        ...
        Envelope<ObjType> Objectenvelope =
            new(Object, new() { SVCompany = postRequestOptions?.Company ?? Company?.Name });
    ...
    /// <inheritdoc/>
    public async Task<ObjType> GetObjectAsync<ObjType>(
        string lookupValue,
        MasterRequestOptions? requestOptions = null,
        CancellationToken token = default
    )
        where ObjType : TallyBaseObject, INamedTallyObject
    {
        ...
        List<Filter> filters =
            new()
            {
                new Filter() { FilterName = "Objfilter", FilterFormulae = filterformulae }
            };

        PaginatedRequestOptions collectionRequestOptions =
            new()
            {
        ...
        List<ObjType>? objects = (
            await GetObjectsAsync<ObjType>(collectionRequestOptions, token)
        )?.Data;
    ...
    public async Task<ObjType> GetObjectAsync<ObjType>(
        string lookupValue,
        VoucherRequestOptions? requestOptions = null,
        CancellationToken token = default
    )
        where ObjType : Voucher
    {
        // If received FetchList in collectionOptions we will use that else use default fetchlist
        requestOptions ??= new();
        requestOptions.FetchList ??= Constants.DefaultFetchList;
        ...
        {
            filterformulae =
                $"${requestOptions.LookupField} = \"{lookupValue.Replace("\"", "\" + $$StrByCharCode:34 + \"")}\"";
        }
        List<Filter> filters =
            new()
            {
        ...
        PaginatedRequestOptions paginatedRequestOptions =
            new()
            {
                FetchList = requestOptions.FetchList,
                Filters = filters,
        ...
        List<ObjType>? objects = (
            await GetObjectsAsync<ObjType>(paginatedRequestOptions, token)
        )?.Data;
        if (objects != null && objects.Count > 0)
        {
            return objects[0];
    ...
    /// <inheritdoc/>
    public async Task<PaginatedResponse<ObjType>?> GetObjectsAsync<ObjType>(
        PaginatedRequestOptions? objectOptions = null,
        CancellationToken token = default
    )
        where ObjType : TallyBaseObject
    {
        ...
        CollectionRequestOptions collectionOptions =
            new()
            {
                ...
                FetchList =
                    (objectOptions?.FetchList) != null
                        ? new(objectOptions.FetchList)
                        : new() { "MasterId", "CanDelete", "*" },
                ...
                ComputeVar =
                    (objectOptions?.ComputeVar) != null ? new(objectOptions.ComputeVar) : null,
        ...
        var mapping = Mappings.TallyObjectMappings.FirstOrDefault(map =>
            map.MasterType == tallyObjType
        );
        ...
        try
        {
            collectionOptions.XMLAttributeOverrides.Add(
                typeof(Colllection<ObjType>),
                "Objects",
                attrs
            );
        }
        ...
        var paginatedData = await GetCustomCollectionAsync<ObjType>(
            collectionOptions,
            $"Getting {mapping?.MasterType} from Tally",
            token
        );
        paginatedData?.Data?.ForEach(obj =>
        {
            ...
            {
                AliasObj.Alias =
                    AliasObj.LanguageNameList != null && AliasObj.LanguageNameList.Count >= 1
                        ? AliasObj.LanguageNameList?.First()?.LanguageAlias
                        : string.Empty;
            }
        });
    ...
    /// <inheritdoc/>
    public async Task<List<ObjType>> GetAllObjectsAsync<ObjType>(
        RequestOptions? objectOptions = null,
        IProgress<ReportProgressHelper>? progress = null,
        CancellationToken token = default
    )
        where ObjType : TallyBaseObject
    {
        XmlRootAttribute? RootAttribute = (XmlRootAttribute?)
            Attribute.GetCustomAttribute(typeof(ObjType), typeof(XmlRootAttribute));
        ...
        CollectionRequestOptions collectionOptions =
            new()
            {
                ...
                ToDate = objectOptions?.ToDate,
                FetchList =
                    (objectOptions?.FetchList) != null ? new(objectOptions.FetchList) : null,
                Compute = (objectOptions?.Compute) != null ? new(objectOptions.Compute) : null,
                ComputeVar =
                    (objectOptions?.ComputeVar) != null ? new(objectOptions.ComputeVar) : null,
                Objects = objectOptions?.Objects,
        ...
        var mapping = Mappings.TallyObjectMappings.FirstOrDefault(map =>
            map.TallyMasterType.Equals(
                collectionOptions.CollectionType,
        ...
        do
        {
            ...
            if (mapping != null)
            {
                _logger?.LogInformation(
                    "Received {type} from {start} to {end} (Page {cur} of {Total})",
                    mapping?.MasterType,
            ...
            progress?.Report(
                new(paginatedResp!.TotalCount, options.RecordsPerPage ?? 0, objects.Count)
            );
    ...
    /// <inheritdoc/>
    public async Task<int?> GetObjectCountAync(
        CountRequestOptions options,
    ...
    )
    {
        ...
        requestEnvelope.Body.Desc.TDL.TDLMessage.Collection.Add(
            new(
                CollectionName,
                options.CollectionType,
                options.ChildOf,
                filters: options
                    .Filters?.Where(f => !f.ExcludeinCollection)
                    .Select(c => c.FilterName)
                    .ToList()!
            )
        );

        options.Filters?.ForEach(filter =>
            requestEnvelope.Body.Desc.TDL.TDLMessage.System?.Add(
                new(name: filter.FilterName!, text: filter.FilterFormulae!)
            )
        );
        ...
        {
            Envelope<string>? Envelope = XMLToObject.GetObjfromXml<Envelope<string>>(
                Response.Response,
                null,
                _logger
            );
    ...
    /// <inheritdoc/>
    public async Task<PaginatedResponse<ObjType>?> GetCustomCollectionAsync<ObjType>(
        CollectionRequestOptions collectionOptions,
        string? requestType = null,
        CancellationToken token = default
    )
        where ObjType : TallyBaseObject
    {
        ...
        if (Response.Status == RespStatus.Sucess && Response.Response != null)
        {
            try
            {
                Envelope<ObjType>? Envelope = XMLToObject.GetObjfromXml<Envelope<ObjType>>(
                    Response.Response,
                ...
                {
                    v =
                        await GetObjectCountAync(
                            new CountRequestOptions()
                            {
                ...
                return new PaginatedResponse<ObjType>(
                    v,
                    collectionOptions.RecordsPerPage ?? 1000,
    ...
    /// <inheritdoc/>
    public async Task<string> GenerateCollectionXML(
        CollectionRequestOptions collectionOptions,
        bool indented = false
    )
    {
        StaticVariables staticVariables =
            ...
            {
                SVCompany =
                    collectionOptions.Company
                    ?? Company?.Name
                    ?? await GetActiveSimpleCompanyNameAsync(),
                SVFromDate = collectionOptions.FromDate ?? Company?.BooksFrom,
                SVToDate =
                    collectionOptions.ToDate
                    ?? (collectionOptions.FromDate == null ? null : DateTime.Now),
            };
        ...
        ColEnvelope.Body.Desc.TDL.TDLMessage = new(
        ...
        if (collectionOptions.Pagination)
        {
            ...
            ColEnvelope.Body.Desc.TDL.TDLMessage.Collection.Add(
                new()
                {
                    ...
                    ComputeVar = new()
                    {
                        "vLineIndex: Number : IF $$IsEmpty:##vLineIndex THEN 1 ELSE ##vLineIndex + 1"
                    },
                    NativeFields = new() { "*" },
            ...
            ColEnvelope.Body.Desc.TDL.TDLMessage.System?.Add(
                new(
                    "PaginationFilter",
    ...
    /// <inheritdoc/>
    public async Task<ReturnType?> GetTDLReportAsync<ReportType, ReturnType>(
        DateFilterRequestOptions? requestOptions = null,
        string? requestType = null,
        CancellationToken token = default
    )
    {
        StaticVariables sv =
            new()
            {
                SVCompany =
                    ...
                    ?? Company?.Name
                    ?? await GetActiveSimpleCompanyNameAsync(token),
                SVExportFormat = "XML",
        ...
        TallyResult Response = await SendRequestAsync(
            xml: Reqxml,
    ...
    /// <inheritdoc/>
    public async Task<ReturnType?> GetTDLReportAsync<ReturnType>(
        DateFilterRequestOptions? requestOptions = null,
        CancellationToken token = default
    )
        where ReturnType : TallyBaseObject
    {
        return await GetTDLReportAsync<ReturnType, ReturnType>(
    ...
    /// <inheritdoc/>
    public async Task<TallyResult> SendRequestAsync(
        string? xml = null,
        string? requestType = null,
        CancellationToken token = default
    )
    ...
    private static string CleanValue(System.Text.RegularExpressions.Match c)
    {
        ...
        {
            string v = XmlTextHexRegex.Replace(
                value,
                innerValue =>
                {
    ...
    private static string CleanXml(System.Text.RegularExpressions.Match c)
    {
        ...
        {
            string v = XmlTextAsciiRegex.Replace(
                value,
                innerValue =>
                {
                    var innerText = innerValue.Value;
    ...
    public TallyResult ParseResponse(TallyResult tallyResult)
    {
        ...
        {
            ResponseEnvelope? Resp = XMLToObject.GetObjfromXml<ResponseEnvelope>(
                tallyResult.Response!
            ); //Response from tally on sucess
            ...
            if (Resp?.Body?.Data?.ImportResult != null)
            {
                ...
                if (
                    Resp.Body.Data.ImportResult.LastVchId != null
                    && Resp.Body.Data.ImportResult.LastVchId != 0
                )
                {
                    tallyResult.Response +=
                        ", LastVchId - " + Resp.Body.Data.ImportResult.LastVchId.ToString(); //Returns VoucherMaster ID
                }
            }
        }
        else
        {
            FailureResponse? resp = XMLToObject.GetObjfromXml<FailureResponse>(
                tallyResult.Response!
            ); //Response from tally on Failure
            tallyResult.Status = RespStatus.Failure;
